...
public 
class WebASLogFiles extends HttpServlet 
{
  private static final long serialVersionUID = -5589695216838553146L;
  
  protected static String AS_LOG_FOLDER = "/opt/wildfly/standalone/log";
  
  public
  void init()
    throws ServletException
  {
    File folder = new File(AS_LOG_FOLDER);
    if(!folder.exists()) {
      AS_LOG_FOLDER = App.LOG_FOLDER;
    }
  }
    ...
    throws ServletException, IOException 
  {
    ...
    if (fileName != null && fileName.length() > 0) {
      ...
      out.println("</head>");
      out.println("<body>");
      out.println("<h1>" + AS_LOG_FOLDER + "</h1>");
      out.println("<hr>");
      ...
      for (int i = 0; i < listFiles.size(); i++) {
        ...
        if (file != null) {
          out.println("<td><a href=\"aslog/" + fileNameI + "\">" + fileNameI + "</a></td>");
        }
        else {
  ...
  {
    File file = new File(AS_LOG_FOLDER + File.separator + fileName);
